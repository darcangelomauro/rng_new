...
for s in range(1, n+1):
                ...
                if check_p:
                    ...
                    for l in range(s - sum_k + 1):
                        ...
                        for num_q in range(int(math.pow(2, n-s))):
                            ...
                            for item in second:
                                if item.find('MAT') != -1:
                                    temp_51.append('1-' + item.replace('MAT', 'e'))
                                elif item.find('dM') != -1:
                                    temp_51.append('1-e[uM]')
                            for item in third:
                                if item.find('MAT') != -1:
                                    temp_51.append('1-' + item.replace('MAT', 'e'))
                                elif item.find('dM') != -1:
                            ...
                            first = first_cyclic(first)
                            '''
                            # 6. simplify gammas (gamma[i] squares to identity)
                            check_6 = 1
                            while len(first) > 1 and check_6:
                                check_6 = 0
                                for idx_6 in range(len(first)-1):
                            ...
                            terms.append([str(binom) + '*' + '*'.join(e)] + [first] + [second] + [third])
    ...
    for term in terms:
        for idx_10 in range(1, len(term)):
            check_10 = 1
    ...
    for item in products:
        if len(item) > 1:
            ...
            res = res.replace('[', '_')
            res = res.replace(']', '_')
            print('gsl_complex tr' + res + ' = trace(' + ''.join(item) + ');')
    ...
    for term in terms:
        ...
        print('pfac = gsl_complex_rect(' + pre + ', 0);')
        compl = []
        for idx_12 in range(1, len(term)):
            res = ''.join(term[idx_12])
            res = res.replace('[', '_')
            res = res.replace(']', '_')
            compl.append('tr' + res)
