...
void print_renormalized(FILE* fobsS, FILE* fobsHL, gsl_matrix_complex** MAT_r, int dim_r)
{
    ...
    for(int l=0; l<nHL; l++)
    {
        for(int i=0; i<dim_r; i++)
        {
            ...
            {
                gsl_complex z = gsl_matrix_complex_get(MAT_r[l], i, j);
                fprintf(fobsHL, "%.15lf ", GSL_REAL(z)); 
                fprintf(fobsHL, "%.15lf ", GSL_IMAG(z));
            }
        }
    ...
    for(int i=0; i<nH; i++)
    {
        ...
        gsl_blas_zhemm(CblasLeft, CblasUpper, GSL_COMPLEX_ONE, MAT_r[i], MAT_r[i], GSL_COMPLEX_ZERO, H2_r);
        double tr_r = trace_herm(H_r[i]);
        double tr2_r = trace_herm(H2_r);
        fprintf(fobsS, "%.15lf %.15lf ", tr_r, tr2_r);
